...
def create_acc_msg(packer, CAN: CanBus, long_active: bool, gas: float, accel: float, stopping: bool):
  ...
  brake = accel < -1 or stopping and long_active 
  values = {
    "AccBrkTot_A_Rq": accel,                          # Brake total accel request: [-20|11.9449] m/s^2
    "Cmbb_B_Enbl": 1 if long_active else 0,           # Enabled: 0=No, 1=Yes
    "AccPrpl_A_Rq": -5 if brake else gas,                              # Acceleration request: [-5|5.23] m/s^2
    "AccPrpl_A_Pred": accel,
    "AccResumEnbl_B_Rq": 1 if long_active else 0,
    # TODO: we may be able to improve braking response by utilizing pre-charging better
    "AccBrkPrchg_B_Rq": 1 if brake else 0,                             # Pre-charge brake request: 0=No, 1=Yes
